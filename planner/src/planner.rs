...
pub struct Planner {
    ...
    #[allow(dead_code)] // Reserved for future use to provide conversation context
...
impl Planner {
    ...
    pub fn build_system_prompt(&self, available_tools: &[ToolInfo]) -> String {
        ...
        if available_tools.is_empty() {
            prompt.push_str(
                "No tools are available. You can only use reasoning and response steps.\n\n",
            );
        } else {
    ...
    pub async fn create_plan(&self, goal: &str, available_tools: &[ToolInfo]) -> Result<Plan> {
        ...
        let messages = vec![Message::system(&system_prompt), Message::user(goal)];
    ...
    pub fn parse_plan(&self, response: &str) -> Result<Plan> {
        ...
        let plan: Plan = serde_json::from_str(json_str).map_err(|e| {
            agent_core::AgentError::Planning(format!(
    ...
    fn extract_json<'a>(&self, response: &'a str) -> Result<&'a str> {
        ...
        if let Some(start_marker) = trimmed.find("```json") {
            ...
            if let Some(end_marker) = trimmed[code_start..].find("```") {
                return Ok(trimmed[code_start..code_start + end_marker].trim());
            }
        }
        ...
        if let Some(start_marker) = trimmed.find("```") {
            ...
            if let Some(end_marker) = trimmed[code_start..].find("```") {
                let content = trimmed[code_start..code_start + end_marker].trim();
                // Only accept if it looks like a JSON object
        ...
        // 2. Fallback: Find first '{' followed by '"' (ignoring whitespace)
        // This avoids capturing text like "Use the {tool}..."
        let mut chars = trimmed.char_indices();
        while let Some((i, c)) = chars.next() {
            if c == '{' {
                // Check if next non-whitespace is '"'
                let clone = chars.clone();
                let mut is_json_start = false;
                for (_, next_c) in clone {
        ...
        // 3. Last resort: simple brace finding (for backward compatibility or malformed JSON)
        ...
        Err(agent_core::AgentError::Planning(format!(
        ...
        for step in &plan.steps {
            if let Step::ToolCall(tool_call) = step {
                ...
                if registry.get(&tool_call.tool_name).is_none() {
                    return Err(agent_core::AgentError::Planning(format!(
...
mod tests {
    ...
    impl llm::LLMProvider for MockLLM {
        async fn send_message(&self, _messages: &[Message]) -> Result<String> {
            ...
            let response = self
                ...
                .get(*index)
                .ok_or_else(|| {
                    agent_core::AgentError::LLMProvider(format!(
                        "No response available for call {}",
                        *index
                    ))
                })?
                .clone();
    ...
    fn test_parse_plan_with_json_wrapped_in_text() {
        ...
        let plan = planner
            .parse_plan(response_with_extra_text)
    ...
    fn test_validate_plan_with_multiple_invalid_tools() {
        ...
        assert!(
            result.is_err(),
            "Validation should fail on first invalid tool"
        );
        ...
        let error_msg = error.to_string();
        assert!(
            error_msg.contains("tool1"),
            "Error should mention the first invalid tool: {}",
            error_msg
        );
    }
    ...
    fn test_validate_plan_without_tool_calls() {
        ...
        assert!(
            result.is_ok(),
            "Validation should succeed without tool calls"
        );
    }
    ...
    fn test_build_system_prompt_with_no_tools() {
        ...
        assert!(
            prompt.contains("planning assistant"),
            "Prompt should identify role"
        );
        assert!(prompt.contains("JSON"), "Prompt should mention JSON format");
        assert!(
            prompt.contains("No tools are available"),
            "Prompt should indicate no tools"
        );
        assert!(
            prompt.contains("reasoning") && prompt.contains("response"),
    ...
    fn test_build_system_prompt_with_single_tool() {
        ...
        assert!(
            prompt.contains("arithmetic operations"),
            "Prompt should include tool description"
        );
        assert!(
            prompt.contains("parameters_schema") || prompt.contains("operation"),
    ...
    fn test_build_system_prompt_with_multiple_tools() {
        ...
        assert!(
            prompt.contains("Math operations"),
            "Should include calculator description"
        );
        assert!(
            prompt.contains("Read files"),
            "Should include file_reader description"
        );
        assert!(
            prompt.contains("Search the web"),
            "Should include web_search description"
        );
    }
    ...
    fn test_build_system_prompt_includes_guidelines() {
        ...
        assert!(
            prompt.contains("tool names match"),
    ...
    async fn test_create_plan_with_mock_llm() {
        ...
        let plan = planner
            .create_plan("What is 10 + 20?", &tools)
            .await
    ...
    fn test_extract_json_with_whitespace() {
        ...
        let extracted = planner
            .extract_json(json_with_whitespace)
    ...
    #[test]
    fn test_extract_json_with_markdown_blocks() {
        ...
        let extracted = planner
            ...
            .expect("Should extract from markdown");

        assert!(extracted.contains("\"reasoning\": \"test\""));
        assert!(!extracted.contains("```"));
    }

    #[test]
    fn test_extract_json_with_braces_in_text() {
        // Test that we don't capture braces in the text preamble
        let planner = create_test_planner(vec![]);

        let response =
            "I will use the {calculator} tool. Here is the plan: {\"reasoning\": \"test\"}";
        let extracted = planner.extract_json(response).expect("Should extract JSON");

        assert_eq!(extracted, "{\"reasoning\": \"test\"}");
    }
}
