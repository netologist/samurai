...
async fn test_tool_execution_with_division_by_zero() {
    ...
    assert!(!step_result.success);
    assert!(
        step_result.output.contains("Division by zero") || step_result.output.contains("division")
    );
}
...
async fn test_file_reader_with_nonexistent_file() {
    ...
    assert!(!step_result.success);
    assert!(
        step_result.output.contains("File not found")
...
async fn test_tool_execution_with_unknown_tool() {
    ...
    assert!(!step_result.success);
    assert!(
        step_result.output.contains("not found") || step_result.output.contains("Tool not found")
    );
}
...
async fn test_tool_parameter_validation() {
    ...
    assert!(!result.step_results[0].success);
    assert!(
        result.step_results[0].output.contains("Unknown operation")
...
async fn test_complex_multi_tool_workflow() {
    ...
    assert_eq!(
        ...
        "Workflow completed successfully with all tools"
    );
